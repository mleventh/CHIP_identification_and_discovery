...
parser.add_argument('-b', '--blacklist', dest='blacklist', metavar='',
        help='Path to list of genes to exclude beofre applying rules')
...
# For annovar headers
func_lookup = {
	"position"	: "position(spl[idx('pos_start') ], spl[idx('pos_end') ], ## )",
	"type" 		: "typemut(spl[idx('result') ], ## )",
	"aa_match" 	: "aa_match(spl[idx('aa') ], ## )",
	"aa_range"	: "aa_range(spl[idx('aa') ], ## )",
	"fs" 		: "frameshift(spl[idx('result') ], ## )",
	"field" 	: "fieldmatch(line, ## )",
	"ExAC_Freq"	: "exac_freq(spl[idx('ExAC_Freq')] , ##)"
}

# Dictionaries to be laoded
rules = {  
...
def main():
	...
	# print blacklist
	# NEED BETTER FILE PATH HANDLING
	# - Load blacklist
	...
	#selectpath = os.path.join(outdir, selectfile)
	dropfile = outfile + "_dropped.txt"
	droppath = dropfile
	#droppath = os.path.join(outdir, dropfile)
	...
	if (bg == True):
		...
		for g in rules:
			spath = os.path.join(gene_dir, g + "_" + selectfile)
			dpath = os.path.join(gene_dir, g + "_" + dropfile)
	...
	try:
		with open(infile) as f:		
			...
			for fi in gene_outfiles.values() + [(sel, drp)]:
			...
			for line in f:
				...
				var = ( spl[idx('chrom')], spl[idx('pos_start')], 
						spl[idx('pos_end')], spl[idx('ref')], spl[idx('var')]
					)
				...
				try:
					...
					r = rules[spl[idx('gene')]]
					if eval(r): 
						sel.write(line)
						if bg == True:
							gene_outfiles[spl[idx('gene')]][0].write(line)
					...
					# Else they don't match print to discarded file
						...
						if bg == True:
							gene_outfiles[spl[idx('gene')]][1].write(line)

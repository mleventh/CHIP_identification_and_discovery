...
DZ_IDX = ["TG_C_T","TA_C_T","TC_C_T","TT_C_T","TG_C_A","TA_C_A","TC_C_A","TT_C_A","TG_C_G","TA_C_G","TC_C_G","TT_C_G",
          ...
          "TG_G_T","TA_G_T","TC_G_T","TT_G_T","TG_G_A","TA_G_A","TC_G_A","TT_G_A","TG_G_C","TA_G_C","TC_G_C","TT_G_C",
          "CG_G_T","CA_G_T","CC_G_T","CT_G_T","CG_G_A","CA_G_A","CC_G_A","CT_G_A","CG_G_C","CA_G_C","CC_G_C","CT_G_C",
          "AG_G_T","AA_G_T","AC_G_T","AT_G_T","AG_G_A","AA_G_A","AC_G_A","AT_G_A","AG_G_C","AA_G_C","AC_G_C","AT_G_C",
          "GG_G_T","GA_G_T","GC_G_T","GT_G_T","GG_G_A","GA_G_A","GC_G_A","GT_G_A","GG_G_C","GA_G_C","GC_G_C","GT_G_C",
          "TG_T_G","TA_T_G","TC_T_G","TT_T_G","TG_T_C","TA_T_C","TC_T_C","TT_T_C","TG_T_A","TA_T_A","TC_T_A","TT_T_A",
          "CG_T_G","CA_T_G","CC_T_G","CT_T_G","CG_T_C","CA_T_C","CC_T_C","CT_T_C","CG_T_A","CA_T_A","CC_T_A","CT_T_A",
          "AG_T_G","AA_T_G","AC_T_G","AT_T_G","AG_T_C","AA_T_C","AC_T_C","AT_T_C","AG_T_A","AA_T_A","AC_T_A","AT_T_A",
          "GG_T_G","GA_T_G","GC_T_G","GT_T_G","GG_T_C","GA_T_C","GC_T_C","GT_T_C","GG_T_A","GA_T_A","GC_T_A","GT_T_A"]

DZ_IDX_SET = set(DZ_IDX)

DZ_BLEED_CONTEXT = ["TG_C_T","TA_C_T","TC_C_T","TT_C_T","TG_C_G",
                    "CG_C_G",
...
DZ_BLEED_SET = set(DZ_BLEED_CONTEXT)
...
class VCFReader(object):
    ...
    def __init__(self, filename):
        ...
        line = next(self.__reader).decode("utf-8")
...
def parse_cl_args():
    ...
    parser.add_argument("-n","--nullfile", help="file without sequencing bleed",required=True)
...
def gen_lego_plot(dz, basename):
    ...
    for idx in range(len(xpos)):
        ax.bar3d(xpos[idx], ypos[idx], zpos[idx], dx[idx], dy[idx], dz[idx], 
                 edgecolor="black", linewidth=0.5, color=df.barCol.tolist()[idx], zsort="max")
    ...
    for idx, pos in enumerate(range(96,112)):
        ax.text(xpos[pos]+0.45, ypos[pos]+0.45, 0, CONT_LIST[idx], size="small", ha="center", va="center", zorder=pos+16)#112)  
...
def gen_dz_arr_from_maf(muts_df, cc, rc, ac):
    dz_arr = pd.Series(data=np.zeros(192), index=DZ_IDX, dtype=np.int64)
...
def main():
    ...
    if file_type == "vcf":
        ...
        null_df = vcf_to_muts_df(null_file, ref_fasta_file, ref_col, alt_col, context_col, af_col, judge_col)
    else:
        try:
            muts_df = pd.read_table(muts_file, comment="#")
            null_df = pd.read_table(null_file, comment="#")
    ...
    if af_cut < 1.0:
        ...
        muts_df_lowAF = muts_df.ix[muts_df[af_col] <= af_cut,]

    dz_bleed_null=pd.read_csv(null_file,sep='\t',index_col=0)

    quantile_map=dict(zip(DZ_BLEED_CONTEXT,dz_bleed_null.Quantile))

    dz = gen_dz_arr_from_maf(muts_df_lowAF, context_col, ref_col, alt_col)
    dz_dict = dict(zip(DZ_IDX,dz))

    dz_dict = {k: dz_dict[k] for k in quantile_map.keys()}
    dz_bleed={k: v for k, v in dz_dict.items() if v>quantile_map[k]}

    bleed_contexts=list(dz_bleed.keys())

    muts_df_lowAF["cont_ext"]=muts_df_lowAF["cont_ext"] + "_" + muts_df_lowAF["Reference_Allele"].str.upper() + "_" + muts_df_lowAF["Tumor_Seq_Allele2"].str.upper()

    muts_df["cont_ext"] = muts_df["ref_context"].apply(extract_context)
    muts_df["cont_ext"]=muts_df["cont_ext"] + "_" + muts_df["Reference_Allele"].str.upper() + "_" + muts_df["Tumor_Seq_Allele2"].str.upper()

    muts_df_AF_filtered=muts_df_lowAF.ix[muts_df_lowAF.cont_ext.isin(bleed_contexts),]

    muts_df_filtered=pd.concat([muts_df,muts_df_AF_filtered],axis=0)
    muts_df_filtered.drop_duplicates(keep=False,inplace=True)

    muts_df_filtered.to_csv(basename+"_bleedthrough_filtered.maf",sep='\t',index=False)
    muts_df_AF_filtered.to_csv(basename+"_bleedthrough_failed.maf",sep='\t',index=False)

    writer=open("num_passed.txt",'w')
    num_pass=len(muts_df_filtered.index)
    writer.write(str(num_pass))
    writer.close()

    writer=open("num_rejected.txt",'w')
    num_rejected=len(muts_df.index)-len(muts_df_filtered.index)
